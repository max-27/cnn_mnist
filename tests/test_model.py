...
@pytest.mark.parametrize(
    "data_sample, cnn_output_size",
    [
        (torch.randint(0, 255, size=(1, 28, 28)), 196),
...
def test_model(data_sample, cnn_output_size) -> None:
    ...
    assert output_cnn.shape == torch.Size([cnn_output_size]), (
        f"Expected output of flatten cnn layer " f"{cnn_output_size} but got {output_cnn.shape} "
    )
